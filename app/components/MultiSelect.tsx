...
>(({ className, children, ...props }, ref) => {
  const { value, data, onValueChange, activeIndex } = useMultiSelect();
...
>(({ children }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("relative", { hidden: !open })}>
      {children}
    </div>
  );
});
...
>(({ className, children }, ref) => {
  return (
    ...
    >
      {children}
      <CommandEmpty>
        <span className="text-muted-foreground">אין תוצאות</span>
      </CommandEmpty>
    </CommandList>
  );
});
