  ...
  return (
    <>
      <Form method="GET" className="flex w-full flex-col space-y-4 p-4">
        ...
        <h2>Status</h2>
        <div className="flex gap-3 items-center">
          <Checkbox type="submit" name="filter" value={"Status:1"} />
          <CircleCheckBig className="text-green-400 size-5" /> Completed
        </div>
        <div className="flex gap-3 items-center">
          <Checkbox type="submit" name="filter" value={"Status:0"} />
          <LoaderCircle className="size-5" /> Waiting
        </div>
        <h2>Members</h2>
        {users?.map((user, index) => (
          <div key={index} className="flex gap-2">
            <Checkbox type="submit" name="filter" value={`Members:${user.id}`} />
